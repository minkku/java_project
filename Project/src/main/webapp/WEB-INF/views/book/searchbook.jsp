					...
					<li><a
						href='/book/bookdetail?books_id=<c:out value="${book.books_id}"/>'>
							<span class="thumb"> <img
								src="../resources/images/${book.img}" alt=""> <em>${book.name}</em>
						</span> <strong>${book.author}</strong>
					</a>
						<p>${book.price}</p></li>
				</c:forEach>
				<li>
		...
		<ul class="pagination">
			<c:forEach begin="1" end="${totalPages}" varStatus="loop">
				<li class="${loop.index == currentPage ? 'active' : ''}">
					<c:url	value="/book/search" var="paginationUrl">
						<c:param name="page" value="${loop.index}" />
						<c:param name="searchType" value="${param.searchType}" />
						<c:param name="keyword" value="${param.keyword}" />
					</c:url> <a href="${paginationUrl}" data-page="${loop.index}">${loop.index}</a>
				</li>
			</c:forEach>
		</ul>
	</div>
